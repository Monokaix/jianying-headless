...
- **删除/覆盖任何已存在的草稿文件夹之前**（不只是 publish 之前），必须紧挨着
  那个删除动作再检查一次剪映有没有在运行，不能只信任更早时候检查过的结果——
  用户可能在两次操作之间的空隙重新打开剪映。真实事故：删除时剪映还开着，
  用户在上一次发布后手动加的内容（水印、模板）连同剪映自己的备份一起被删掉，
  永久丢失。剪映在运行就停下来问用户，不要自己决定跳过或稍后再查。
