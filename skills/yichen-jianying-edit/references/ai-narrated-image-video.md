...
`target_duration_us` 是第 3 步从草稿里读出来的音频片段目标时长(剪映批量朗读
自己算出来的、应用了变速系数之后的时长)。**这个字段应该总是提供**——省略会
让脚本按原始语音自然时长(未变速)播放，跟用户在剪映里实际听到、认可的节奏
对不上，曾经因为怀疑"变速导致变调"而错误地默认省略过这个字段，排查后发现
变速本身不是问题根源(plan.json 里设置的 speed 字段和剪映内部用的是同一套
机制，JSON 设置与 UI 拖拽应该是同一效果)，真正的音色细微差异更可能来自于
每次在剪映里重新生成朗读都是全新的一次 TTS 合成，本身就会有一点声音差异，
不是这条链路引入的。所以除非明确要自然语速，否则务必带上 `target_duration_us`。
...
- **每次删除/重建草稿之前，紧挨着那条 `rm -rf` 命令本身，必须先跑
  `python3 SKILL/scripts/require_jianying_closed.py`，它返回非零就不准继续，
  必须停下来提示用户完全退出剪映(Cmd+Q，不是切后台)再重试。** 这是硬性规则，
  不是建议——在对话早些时候检查过一次剪映没开不够，用户随时可能在两次
  操作之间的空隙重新打开它。曾经真实发生过两次事故：(1) 剪映还开着的时候
  `rm -rf` 删除旧草稿文件夹，剪映重新打开时在原路径新建了一个空白项目顶替，
  内容全丢；(2) 用户在上一次发布之后手动在剪映里加了 4K 水印和模板内容，
  这条链路不知道用户加过东西，紧接着又做了一次删除重建，`rm -rf`
  把整个文件夹（连同剪映自己的 `.backup/` 快照）一起删了，手动加的内容
  永久丢失、无法恢复。**删除前只要发现剪映在运行，就必须停下来问用户，
  而不是自己决定等一下再删或者直接跳过检查。**
